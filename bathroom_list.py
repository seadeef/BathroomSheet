...
def register_id(student_id):
	if student_id not in timestamps:
		...
		if len(timestamps) > 0:
			sorted_dict = sorted(timestamps.items(), key=lambda x: x[1].timestamp(), reverse=True)
			return ('register', sorted_dict[0][0])
		return ('unregister', student_id)
