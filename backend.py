...
def index():
    ...
    status = None if len(msgs) == 0 else msgs[0]
    student_id = None if len(msgs) < 2 else msgs[1]

	# Render
    return render_template('index.html', status=status, student_id=student_id, alert_threshold=ALL["ALERT_THRESHOLD"], teacher_name=ALL["TEACHER_NAME"])
